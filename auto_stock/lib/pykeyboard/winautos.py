...
from auto_stock.common import VK_CODE, PyKeyboard
from auto_stock.lib.comm import writTxt
...
class Winauto(object):
    ...
    def BuyS(self, buy_stock, buy_price, buy_nun):
        ...
        time.sleep(2)
        if buy_price != '0':
            self.k.type_string(buy_nun)  # k.type_string(buy_price)
        win32api.keybd_event(VK_CODE['tab'], 0, 0, 0)  #
        self.k.type_string(buy_nun)  #
        win32api.keybd_event(VK_CODE['enter'], 0, 0, 0)  #  # win32api.keybd_event(VK_CODE['enter'],0,0,0) #
